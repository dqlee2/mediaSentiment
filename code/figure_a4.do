...
log using "..\log\figure_a4_`logdate'.txt", text append
version 14
set linesize 225

********************************************************************************
*** Figure A4. Benchmark results - Country and Time Split ***
********************************************************************************
...
* Figure A.4. Benchmark Results -- Country and Time Split

xtset weo DATE
foreach tt in 91_99 99_07 10_15 { 
foreach cc in AE EM {
foreach DV in index {
foreach s in 1 {
...
foreach DFM in factor_sent`s'_best_filter {
	...
	if `"`tt'"'=="91_99" {
		loc cnd_tt = "& tin(1jan1991, 1jan1999)"
	}
	if `"`tt'"'=="99_07" {
		loc cnd_tt = "& tin(1jan1999, 1sep2007)"
	}
	if `"`tt'"'=="10_15" {
...
preserve
	...
	* One regression for each horizon of the response
	...
	esttab * using ///
		"..\results\figure_a4_`tt'_`cc'.csv" ///
...
***** create confidence bands (in this case 90 and 95%) *****
	...
	graph export ///
		"..\results\figure_a4_`tt'_`cc'.pdf" ///
		, as(pdf) replace 
	cap graph close

	save "..\results\figure_a4_`tt'_`cc'.dta", replace
restore
