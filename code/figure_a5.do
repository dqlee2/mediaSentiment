...
log using "..\log\figure_a5_`logdate'.txt", text append
version 14
set linesize 225

********************************************************************************
*** Figure A5. Panel controlling for US EPU ***
********************************************************************************
...
preserve
	...
	keep weo* iso DATE AE F*_`DV' ${L_`impulse'} ${L_`DFM'} ///
				...
				${L_`art_cntrl'} ${L_`un'} ${L_commo} ${L_R_DJ_global} ///
				${other_dummies}
	...
	* One regression for each horizon of the response
	...
	forvalues i=0/$horizon {
		...
		* Store coefficient and se on first lag
		...
		replace b_LP3 = _b[L1_`un'] if _n==`i'+2
		replace se_LP3 = _se[L1_`un'] if _n==`i'+2
	}
	esttab * using ///
		"..\results\figure_a5.csv" ///
...
***** create confidence bands (in this case 90 and 95%) *****
	...
	graph export ///
		"..\results\figure_a5.pdf" ///
		, as(pdf) replace 
	cap graph close

	save "..\results\figure_a5.dta", replace
restore
