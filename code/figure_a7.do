...
log using "..\log\figure_a7_`logdate'.txt", text append
version 14
set linesize 225

********************************************************************************
*** Figure A7. Panel controlling for Citi Index of Economic Surprises ***
********************************************************************************
...
foreach DFM in factor_sent`s'_best_filter {
	...
	if `"`es'"'=="usd" {
		loc cnd_es = "${L_cesi_usd}"
	}
	if `"`es'"'=="g10" {
		loc cnd_es = "${L_cesi_usd} ${L_cesi_g10}"
	}
	if `"`es'"'=="all" {
		loc cnd_es = "${L_cesi_usd} ${L_cesi_eur} ${L_cesi_cny} ${L_cesi_g10}"
	}
...
preserve
	...
	***** create confidence bands (in this case 90 and 95%) *****
	...
	graph export ///
		"..\results\figure_a7.pdf" ///
		, as(pdf) replace 
	cap graph close
	
	save "..\results\figure_a7.dta", replace
restore
